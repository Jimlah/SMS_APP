<div class="flex items-center justify-start space-x-2 ">
    ...
    <div class="static flex" x-data="modal()">
        ...
        <div class="absolute top-0 left-0 flex items-center justify-center w-full h-full bg-purple-700 bg-opacity-25"
            :class="{ 'hidden': isOpen() === false }">
            <div class="flex flex-col items-center justify-center p-5 space-y-5 text-lg text-black bg-white rounded-md"
                ...
                <span>
                    <form action="{{ $delete }}" method="post"
                        class="flex space-x-2 text-sm font-bold text-white">
                        {{ method_field('DELETE') }}
                        {{ csrf_field() }}
